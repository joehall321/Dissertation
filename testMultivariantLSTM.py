...
def plotPredictions(list_predications_truths, model_name):
    pass

def scaleData():
    pass

def inverseScaledData():
    pass
...
for movement in data_movements:
    ...
    frames = data_formatter.getMovementTrials(movement, data_movements)
    print()
    print("Validating "+str(len(frames))+" "+movement+" trials")
    ...
    trial_data, _ = data_formatter.collectFormatData(frames)
    ...
    testX, testY = data_formatter.formatDataSamples(trial_data, sample_size)
    ...
    prediction = pd.DataFrame(index=range(pred_y.shape[0]),columns=range(trial.shape[1]))
    truth = pd.DataFrame(index=range(pred_y.shape[0]),columns=range(trial.shape[1]))
    ...
    lengths = [t.get("total_frames") for t in data_movements[movement]]
    trial_lengths = [sum(lengths[0:idx+1]) for idx in range(len(lengths)-1)]
    ...
    fig.suptitle(str(len(frames))+" "+movement+" trials")
    ...
    axis[0,1].title.set_text('ground_force1_vz')
    axis[0,1].plot(truth[53], label='Truth')
    axis[0,1].plot(prediction[53], label='LSTM prediction')
    axis[0,1].legend()
    ...
    axis[1,1].title.set_text('ground_force2_vz')
    axis[1,1].plot(truth[56], label='Truth')
    axis[1,1].plot(prediction[56], label='LSTM prediction')
    axis[1,1].legend()
